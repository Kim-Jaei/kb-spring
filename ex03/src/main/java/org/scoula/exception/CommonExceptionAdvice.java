package org.scoula.exception;

import lombok.extern.log4j.Log4j;
import org.springframework.http.HttpStatus;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.ControllerAdvice;
import org.springframework.web.bind.annotation.ExceptionHandler;
import org.springframework.web.bind.annotation.ResponseStatus;
import org.springframework.web.servlet.NoHandlerFoundException;

@ControllerAdvice // 모든 컨트롤러에서 발생하는 예외를 처리해줌
@Log4j
// 사실상 컨트롤러
public class CommonExceptionAdvice {

    // 모든 예외 처리(500), 예외가 발생하면 호출된다
    @ExceptionHandler(Exception.class)
    public String expect(Exception ex, Model model) { // 컨트롤러이기 때문에 return값 동일 == view 이름
        log.error("Exception", ex); // 예외 정보를 로그에 기록
        model.addAttribute("exception", ex); // 예외 정보를 모델에 추가
        log.error(model); // 모델 객체를 로그에 출력 (유용하진 않음ㅋㅋ)
        return "error_page"; // 에러 페이지 표시!
    }

    // 404 에러 처리
    @ExceptionHandler(NoHandlerFoundException.class)
    @ResponseStatus(HttpStatus.NOT_FOUND) // 404 상태 코드 반환
    public String handl404(NoHandlerFoundException ex){
        return "custom404";
    }
}
